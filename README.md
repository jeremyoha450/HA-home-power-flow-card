...
Solar paths receive an explicit hidden state when their power is zero, removing the faint yellow wiring from the night-time view even when zero-power lines are enabled. They reappear automatically when solar production resumes.
